...
export async function GET(req: NextRequest) {
  ...
  const scored = dealsWithDistance.map((deal) => {
    ...
    const totalDuration = new Date(deal.expires_at).getTime() - new Date(deal.starts_at || deal.created_at).getTime();
    const timeRemaining = Math.max(0, new Date(deal.expires_at).getTime() - Date.now());
    const urgencyScore = totalDuration > 0 ? Math.max(0, 1 - timeRemaining / totalDuration) : 0.5;
  ...
  switch (tab) {
    case "nearby": scored.sort((a, b) => a.distance - b.distance); break;
    case "trending": scored.sort((a, b) => b.live_viewers - a.live_viewers); break;
    case "expiring": scored.sort((a, b) => new Date(a.expires_at).getTime() - new Date(b.expires_at).getTime()); break;
    default: scored.sort((a, b) => b.score - a.score);
  }

  return NextResponse.json({ deals: scored, meta: { total: scored.length, persona, tab } });
}
